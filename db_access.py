...
User = namedtuple(
    "User", "id vk_id username password name ruobr_id marks status")
...
class Database(object):
    ...
    async def get_users(self, status=True):
        ...
        async with aiosqlite.connect(self.database) as db:
            async with db.execute("SELECT * FROM `USERS` WHERE `status` = ?", (status,)) as cursor:
    ...
    async def add_user(self, vk_id, username, password, name, ruobr_id):
        ...
        async with aiosqlite.connect(self.database) as db:
            await db.execute("INSERT INTO `USERS` (`VK_ID`, `USERNAME`, `PASSWORD`, `NAME`, `RUOBR_ID`) VALUES (?, ?, ?, ?, ?)", (vk_id, username, password, name, ruobr_id))
            await db.commit()

    async def update_marks(self, vk_id, marks):
        """Обновляем оценки"""
        async with aiosqlite.connect(self.database) as db:
            await db.execute("UPDATE `USERS` SET `MARKS` = ? WHERE `VK_ID` = ?", (marks, vk_id))
            await db.commit()
    ...
    async def update_status(self, vk_id, status):
        ...
        async with aiosqlite.connect(self.database) as db:
            await db.execute("UPDATE `USERS` SET `STATUS` = ? WHERE `VK_ID` = ?", (status, vk_id))
            await db.commit()
